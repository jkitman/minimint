...
use crate::config::io::{read_server_config, CONSENSUS_CONFIG, PLAINTEXT_PASSWORD};
...
impl FedimintApi {
    ...
    pub async fn run(mut self, mut task_group: TaskGroup) -> anyhow::Result<ServerConfig> {
        // self.run_config_gen(&mut task_group).await?;

        let f = FedimintServer::new(self.cfg.clone(), self.db.clone(), self.settings.registry.clone(), &mut task_group).await.unwrap();

        f.run_consensus(task_group.make_handle()).await;
        //
        // server.run_consensus(task_group.make_handle()).await;
        // self.task_group.spawn_local("consensus", |handle| server.run_consensus(handle)).await;
        // warn!("Consensus Ending");
    ...
    async fn run_config_gen(&self, task_group: &mut TaskGroup) -> anyhow::Result<() > {
        ...
        if let Ok(password) = fs::read_to_string(self.data_dir.join(PLAINTEXT_PASSWORD)) {
            ...
            info!("Setting password from local file");

            if config_gen.start_consensus(ApiAuth(password)).await.is_ok() {
                info!("Configs found locally");
            }
        }

        info!("Starting config gen API");
        ...
        let server_handle = self.spawn_api(rpc_module, 10).await;
    ...
    /// running
    pub async fn run_consensus_api(api: ConsensusApi) -> anyhow::Result<ServerHandle> {
        ...
        let server = ServerBuilder::new()
            ...
            .build(api.cfg.local.api_bind.to_string())
            .await
            .context(format!("Bind address: {}", api.cfg.local.api_bind))
            .expect("Could not start API server");

        let server_handle = server.start(rpc_module).expect("Could not start API server");

        Ok(server_handle)
    }

    // /// Runs the `FedimintServer` which runs P2P consensus
    // async fn run_consensus(&mut self, server: FedimintServer) -> anyhow::Result<()> {
    ...
    //     // }
    //
    //     server.run_consensus(self.task_group.make_handle()).await;
    //     Ok(())
    // }
    ...
    async fn spawn_api<T>(
        ...
        let server_handle = server.start(module).expect("Could not start API server");
